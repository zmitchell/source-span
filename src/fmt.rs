...
impl Style {
    ...
    #[must_use]
    #[cfg(not(feature = "colors"))]
    pub const fn new(underline: char, marker: char) -> Self {
        Self::Custom(underline, marker, ())
    }
...
impl Char {
    ...
    const fn is_span_horizontal(&self) -> bool {
        matches!(self, Self::SpanHorizontal(_))
    }

    #[allow(clippy::trivially_copy_pass_by_ref)]
    const fn is_span_margin(&self) -> bool {
        matches!(self, Self::SpanMargin(_))
    }
}
...
impl CharMap {
    ...
    fn draw_open_line(&mut self, style: &Style, y: usize, start: usize, end: usize) {
        ...
        for x in start..=end {
            ...
            } else if !self.get(x, y).is_span_margin() {
...
impl Formatter {
    ...
    fn draw_line_number(&self, line: Option<usize>, charmap: &mut CharMap, margin_len: usize) {
        if margin_len > 0 {
            charmap.set(margin_len - 2, 0, Char::Margin('|', self.margin_color));
            match line {
                Some(mut line) => {
    ...
    fn draw_line_highlights<M: Metrics>(
        ...
        for i in 0..highlights.len() {
            ...
            if h.span().start.line == line {
                ...
                } else if first_non_whitespace.is_some()
